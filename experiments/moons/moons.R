...
# Dataset: make_moons (n=300, noise=0.05, seed=42)
# =============================================================================
...
make_moons <- function(n_samples = 300, noise = 0.05, random_state = 42) {
  ...
  # Inner moon (bottom, shifted)
  t2      <- seq(0, pi, length.out = n)
  inner_x <- 1 - cos(t2)
  inner_y <- 1 - sin(t2) - 0.5
  
  X <- rbind(cbind(outer_x, outer_y),
...
cat("Generating Two Moons (n=300, noise=0.05, seed=42)...\n")
...
# Export DBSCAN labels for Python
write.csv(data.frame(db_label = db_labels),
          file.path(path.expand(out_dir), "r_dbscan_labels.csv"),
          row.names = FALSE)

cat("Running K-Means (k=2, nstart=10, seed=42)...\n")
set.seed(42)
km_result <- kmeans(X, centers = 2, nstart = 10, iter.max = 300)
km_labels <- as.integer(km_result$cluster) - 1L
cat("  -> cluster sizes — 0:", sum(km_labels == 0),
    " 1:", sum(km_labels == 1), "\n")

# Export K-Means labels + centroids for Python
...
# =============================================================================
# DISCO SCORES  (using R labels — Python labels loaded after Python runs)
# =============================================================================
...
p1 <- ggplot(plot_df, aes(x1, x2, color = true_label)) +
  ...
  labs(title = "Ground Truth", subtitle = "Two Moons  |  n=300  noise=0.05  seed=42") +
...
title_grob <- grid::textGrob(
  "Two Moons: DBSCAN vs K-Means — DISCO Evaluation",
...
cat("Done.\n")
cat("\n--- Files exported for Python ---\n")
cat("  moons_data_points.csv    <- load this in Python\n")
cat("  r_dbscan_labels.csv      <- R DBSCAN labels\n")
cat("  r_kmeans_labels.csv      <- R K-Means labels\n")
cat("  r_kmeans_centroids.csv   <- R K-Means centroids\n")
cat("  disco_scores_full.csv    <- per-point DISCO scores\n")