# =============================================================================
# DISCO Analysis: 3-Blob Dataset — DBSCAN vs K-Means
# Dataset: Generated Gaussian blobs (n=450, 3 clusters)
#   Cluster 0 — top-left  (blue)
#   Cluster 1 — top-right (orange)
#   Cluster 2 — bottom    (green)
#
# WORKFLOW:
#   Step 1 — Run this script  -> generates data -> exports blobs_data_points.csv
#   Step 2 — Run blobs.ipynb  -> runs DBSCAN + KMeans -> exports label CSVs
#   Step 3 — Run this script  -> loads Python labels -> computes DISCO scores
# =============================================================================

required_pkgs <- c("dbscan", "FNN", "ggplot2", "gridExtra", "MASS")
...
cat("Generating 3-blob dataset (n=450, std=0.55, seed=42)...\n")

set.seed(42)
centres <- list(
  c(1.5, 7.0),   # cluster 0 — top-left  (blue)
  c(6.5, 7.0),   # cluster 1 — top-right (orange)
  c(4.0, 2.5)    # cluster 2 — bottom    (green)
)
n_per  <- 150
std    <- 0.55
sigma  <- diag(2) * std^2
...
X <- do.call(rbind, lapply(centres, function(mu)
  MASS::mvrnorm(n = n_per, mu = mu, Sigma = sigma)
))
y_true <- rep(0:2, each = n_per)
...
# =============================================================================
# EXPORT DATA CSV  (shared with Python notebook)
# =============================================================================

out_dir <- "~/Desktop/R-projects/Disco-R/experiments/blobs"
dir.create(path.expand(out_dir), showWarnings = FALSE, recursive = TRUE)

write.csv(
  data.frame(x1 = X[,1], x2 = X[,2], true_label = y_true),
  file.path(path.expand(out_dir), "blobs_data_points.csv"),
  row.names = FALSE
)
cat("  -> blobs_data_points.csv exported\n\n")

# =============================================================================
# STEP 2 — LOAD PYTHON LABELS
# Run blobs.ipynb first, then come back here
# =============================================================================

cat("Loading Python DBSCAN labels (eps=0.5, min_samples=5)...\n")
...
CLUSTER_COLS <- c(
  "-1" = "#888888",
  "0"  = "#4C9BE8",   # blue   — top-left
  "1"  = "#F4831F",   # orange — top-right
  "2"  = "#50C878"    # green  — bottom
)
...
p1 <- ggplot(plot_df, aes(x, y, color = true_label)) +
  ...
  scale_color_manual(values = CLUSTER_COLS,
                     labels = c("0"="Top-left","1"="Top-right","2"="Bottom"),
                     name = "True label") +
  labs(title = "Ground Truth",
       subtitle = sprintf("3-Blob  |  n=%d  |  true classes=%d",
...
p2 <- ggplot(plot_df, aes(x, y, color = dbscan_label, shape = is_noise)) +
  ...
  labs(title = "DBSCAN Clustering",
       subtitle = sprintf("eps=0.5  minPts=5  |  %d clusters  %d noise",
                          n_db_clusters, n_db_noise)) +
  dark_theme +
  theme(legend.position = "bottom", legend.direction = "horizontal")
...
title_grob <- grid::textGrob(
  "3-Blob Dataset: DBSCAN vs K-Means — DISCO Evaluation",
