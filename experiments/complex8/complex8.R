# =============================================================================
# DISCO Analysis: Complex8 — DBSCAN vs K-Means
# Dataset: complex8.arff (n=2551, 8 clusters, varying shape and density)
#
# IMPORTANT WORKFLOW:
#   1. Run complex8.ipynb in Python FIRST  → exports python_dbscan_labels.csv
#                                                      python_km_labels.csv
#   2. Run this R script SECOND            → loads both Python label files
#
# Python is the reference implementation. Both clustering labels are loaded
# from Python to guarantee identical partitions and comparable DISCO scores.
# =============================================================================
...
cat("Loading complex8.arff...\n")
arff_path <- "~/Desktop/R-projects/Disco-R/data/complex8.arff"
...
out_dir <- "~/Desktop/R-projects/Disco-R/experiments/complex8"
dir.create(path.expand(out_dir), showWarnings = FALSE, recursive = TRUE)

write.csv(
  data.frame(x = X[,1], y = X[,2], true_label = y_true),
  file.path(path.expand(out_dir), "complex8_data_points.csv"),
  row.names = FALSE
)
cat("  -> complex8_data_points.csv exported\n\n")

# =============================================================================
# LOAD PYTHON LABELS  (Python is the reference — run Python notebook first)
# =============================================================================

cat("Loading Python DBSCAN labels...\n")
...
CLUSTER_COLS <- c(
  "-1" = "#888888",
  "0"  = "#4C9BE8", "1" = "#F4831F", "2" = "#50C878",
  "3"  = "#FF6B6B", "4" = "#C77DFF", "5" = "#FFD166",
  "6"  = "#06D6A0", "7" = "#EF476F"
)
...
p1 <- ggplot(plot_df, aes(x, y, color = true_label)) +
  ...
  labs(title = "Ground Truth",
       subtitle = "Complex8  |  n=2551  |  8 clusters") +
...
p2 <- ggplot(plot_df, aes(x, y, color = dbscan_label, shape = is_noise)) +
  ...
  labs(title = "DBSCAN Clustering",
       subtitle = sprintf("eps=10  minPts=5  |  %d clusters  %d noise",
                          n_db_clusters, n_db_noise)) +
  dark_theme + theme(legend.position = "bottom", legend.direction = "horizontal")
...
p3 <- ggplot(plot_df, aes(x, y, color = kmeans_label)) +
  ...
  labs(title = "K-Means Clustering", subtitle = "k=8  |  random_state=42") +
  dark_theme + theme(legend.position = "bottom", legend.direction = "horizontal")

p4 <- ggplot(plot_df, aes(x, y, color = disco_db, shape = is_noise)) +
  geom_point(size = 1.2, alpha = 0.9) + DISCO_GRAD +
  scale_shape_manual(values = c("FALSE"=16,"TRUE"=4), guide = "none") +
...
title_grob <- grid::textGrob(
  "Complex8: DBSCAN vs K-Means — DISCO Evaluation",
