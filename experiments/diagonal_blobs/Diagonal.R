# =============================================================================
# DISCO Analysis: Diagonal Blobs — DBSCAN vs K-Means
# Dataset: 3 elongated diagonal clusters (generated, no file needed)
#   Cluster 0 — top    (green)
#   Cluster 1 — middle (blue)
#   Cluster 2 — bottom (orange)
#
# WORKFLOW:
#   1. Run this script once   -> exports diagonal_blobs_data_points.csv
#   2. Run diagonal_blobs.ipynb in Python
#                             -> exports python_dbscan_labels.csv
#                                         python_km_labels.csv
#   3. Run this script again  -> loads Python labels -> computes DISCO
# =============================================================================

required_pkgs <- c("FNN", "ggplot2", "gridExtra", "MASS")
...
# =============================================================================
# GENERATE DATASET
# Elongated diagonal clusters matching the image:
#   strong negative correlation -> streaks going top-right to bottom-left
# =============================================================================

cat("Generating diagonal blobs dataset (n=600, seed=42)...\n")

set.seed(42)

# Covariance: strong negative correlation = diagonal streak shape
sigma <- matrix(c(1.2, -1.0, -1.0, 1.2), nrow = 2)

centres <- list(
  c( 2.5,  5.0),   # cluster 0 — top    (green)
  c( 0.5,  1.8),   # cluster 1 — middle (blue)
  c(-1.5, -1.5)    # cluster 2 — bottom (orange)
)
...
out_dir <- "~/Desktop/R-projects/Disco-R/experiments/diagonal_blobs"
dir.create(path.expand(out_dir), showWarnings = FALSE, recursive = TRUE)

write.csv(
  data.frame(x = X[,1], y = X[,2], true_label = y_true),
  file.path(path.expand(out_dir), "diagonal_blobs_data_points.csv"),
  row.names = FALSE
)
cat("  -> diagonal_blobs_data_points.csv exported\n\n")

# =============================================================================
# LOAD PYTHON LABELS  (Python is the reference — run notebook first)
# =============================================================================

cat("Loading Python DBSCAN labels (eps=0.8, min_samples=5)...\n")
...
cat("Loading Python K-Means labels (k=3, random_state=42)...\n")
...
CLUSTER_COLS <- c(
  "-1" = "#888888",
  "0"  = "#50C878",   # green  — top
  "1"  = "#4C9BE8",   # blue   — middle
  "2"  = "#F4831F"    # orange — bottom
)
...
p1 <- ggplot(plot_df, aes(x, y, color = true_label)) +
  ...
  scale_color_manual(values = CLUSTER_COLS,
                     labels = c("0"="Top","1"="Middle","2"="Bottom"),
                     name = "True label") +
  labs(title = "Ground Truth",
       subtitle = sprintf("Diagonal Blobs  |  n=%d  |  true classes=%d",
...
p2 <- ggplot(plot_df, aes(x, y, color = dbscan_label, shape = is_noise)) +
  ...
  labs(title = "DBSCAN Clustering",
       subtitle = sprintf("eps=0.8  minPts=5  |  %d clusters  %d noise",
                          n_db_clusters, n_db_noise)) +
  dark_theme +
  theme(legend.position = "bottom", legend.direction = "horizontal")
...
title_grob <- grid::textGrob(
  "Diagonal Blobs: DBSCAN vs K-Means — DISCO Evaluation",
...
cat("Done.\n")
cat("\n--- Files exported ---\n")
cat("  diagonal_blobs_data_points.csv\n")
