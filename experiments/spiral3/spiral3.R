# =============================================================================
# DISCO Analysis: 3-Spiral — DBSCAN vs K-Means
# Dataset: 3-spiral.arff (n=312, 3 spiral clusters)
# =============================================================================
...
cat("Loading 3-spiral.arff...\n")
arff_path <- "~/Desktop/R-projects/Disco-R/data/3-spiral.arff"
raw_data  <- foreign::read.arff(path.expand(arff_path))
str(raw_data)

X      <- as.matrix(raw_data[, c("x", "y")])
# Labels are 1, 2, 3 — convert to 0-based to match convention
y_true <- as.integer(as.character(raw_data$class)) - 1L
...
out_dir <- "~/Desktop/R-projects/Disco-R/experiments/spiral3"
dir.create(path.expand(out_dir), showWarnings = FALSE, recursive = TRUE)

write.csv(
  data.frame(x = X[,1], y = X[,2], true_label = y_true),
  file.path(path.expand(out_dir), "spiral3_data_points.csv"),
  row.names = FALSE
)
cat("  -> spiral3_data_points.csv exported\n\n")

# =============================================================================
# CLUSTERING
# =============================================================================

cat("Running DBSCAN (eps=2, minPts=3)...\n")
db_result     <- dbscan::dbscan(X, eps = 2, minPts = 3)
...
p1 <- ggplot(plot_df, aes(x, y, color = true_label)) +
  ...
  labs(title = "Ground Truth",
       subtitle = "3-Spiral  |  n=312  |  3 clusters") +
...
p2 <- ggplot(plot_df, aes(x, y, color = dbscan_label, shape = is_noise)) +
  ...
  labs(title = "DBSCAN Clustering",
       subtitle = sprintf("eps=2  minPts=3  |  %d clusters  %d noise",
                          n_db_clusters, n_db_noise)) +
  dark_theme + theme(legend.position = "bottom", legend.direction = "horizontal")
...
p3 <- ggplot(plot_df, aes(x, y, color = kmeans_label)) +
  ...
  labs(title = "K-Means Clustering", subtitle = "k=3  |  nstart=20  |  seed=42") +
  dark_theme + theme(legend.position = "bottom", legend.direction = "horizontal")

p4 <- ggplot(plot_df, aes(x, y, color = disco_db, shape = is_noise)) +
  geom_point(size = 2.0, alpha = 0.9) + DISCO_GRAD +
  scale_shape_manual(values = c("FALSE"=16,"TRUE"=4), guide = "none") +
...
title_grob <- grid::textGrob(
  "3-Spiral: DBSCAN vs K-Means — DISCO Evaluation",
